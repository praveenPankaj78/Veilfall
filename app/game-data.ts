...
export type Choice = {
  ...
  requiresRelationships?: Partial<
    Record<RelationshipKey, Partial<RelationshipScore>>
  >;
  forbidsRelationshipIntents?: Partial<
    Record<RelationshipKey, RelationshipIntent[]>
  >;
  requiresFlags?: string[];
  showIfAnyFlags?: string[];
  showIfAllFlags?: string[];
  hideIfAnyFlags?: string[];
  showIfRelationshipIntents?: Partial<
    Record<RelationshipKey, RelationshipIntent[]>
  >;
  result: string;
};
...
const relationshipEffects: Record<string, RelationshipEffects> = {
  'flirt-mara': { mara: { attraction: 1, intent: 'interested' } },
  'ask-marker': { mara: { trust: 1, attraction: 0 } },
  'admit-unease': { mara: { trust: 1, attraction: 1 } },
  'answer-lysara': { lysara: { attraction: 1, intent: 'interested' } },
  'trust-mara': { mara: { trust: 1, attraction: 0 } },
  'take-ridge': { mara: { trust: 1, attraction: 0 } },
  'take-tower': { mara: { trust: 1, attraction: 0 } },
  'protect-lysara-secret': { lysara: { trust: 1, respect: 1 } },
  'tell-brann-seed': { lysara: { trust: -1, friction: 1 } },
  'ask-mara-future': { mara: { trust: 1, attraction: 1 } },
  'trust-mara-opening': { mara: { trust: 1, attraction: 0 } },
  'save-mara-ridge': { mara: { trust: 2, respect: 1 } },
  ...
  'c2-name-fear': { mara: { trust: 2, respect: 1 } },
  'c2-kiss-mara': { mara: { trust: 1, attraction: 2, intent: 'exploring' } },
  'c2-choose-mara-friendship': {
    mara: { trust: 2, respect: 1, intent: 'platonic' },
  },
  ...
  'c4-kiss-mara-bridge': {
    mara: { trust: 1, attraction: 2, intent: 'exploring' },
  },
  ...
  'c5-admit-future-with-mara': {
    mara: { trust: 2, attraction: 1, intent: 'committed' },
  },
  'c5-kiss-mara-after-truth': {
    mara: { trust: 1, attraction: 2, intent: 'committed' },
  },
  'c5-admit-fear-to-mara': { mara: { trust: 2, attraction: 0 } },
  'c5-choose-mara-friendship': {
    mara: { trust: 2, respect: 2, intent: 'platonic' },
  },
  'c5-admit-future-with-lysara': {
    lysara: { trust: 2, attraction: 1, intent: 'committed' },
  },
  'c5-kiss-lysara-after-truth': {
    lysara: { trust: 1, attraction: 2, intent: 'committed' },
  },
  'c5-protect-lysara-choice': { lysara: { trust: 2, attraction: 0 } },
  'c5-choose-lysara-friendship': {
    lysara: { trust: 2, respect: 2, intent: 'platonic' },
  },
  'c5-choose-both-friendship': {
    mara: { trust: 1, respect: 1, intent: 'platonic' },
  ...
  'c7-mara-commit-equal': {
    mara: { trust: 2, attraction: 1, intent: 'committed' },
  },
  'c7-mara-duty-first': {
    mara: { trust: 1, friction: 1, intent: 'exploring' },
  },
  'c7-mara-end-romance': { mara: { respect: 1, intent: 'ended' } },
  'c7-mara-choose-friendship': {
    mara: { trust: 1, respect: 1, intent: 'platonic' },
  },
  'c7-lysara-commit-equal': {
    lysara: { trust: 2, attraction: 1, intent: 'committed' },
  },
  'c7-lysara-mission-first': { lysara: { trust: 1, intent: 'exploring' } },
  'c7-lysara-end-romance': { lysara: { respect: 1, intent: 'ended' } },
  'c7-lysara-choose-friendship': {
    lysara: { trust: 1, respect: 1, intent: 'platonic' },
  },
  'c7-ilyra-deepen-bond': {
    ilyra: { trust: 2, attraction: 2, respect: 1, intent: 'exploring' },
  },
  'c7-ilyra-slow-interest': {
    ilyra: { trust: 1, attraction: 1, intent: 'interested' },
  },
  'c7-ilyra-platonic-alliance': {
    ilyra: { trust: 1, respect: 1, intent: 'platonic' },
  },
  ...
  'c8-lysara-name-shared-road': {
    lysara: { trust: 2, attraction: 1, respect: 1 },
  },
...
export function nextRelationships(
  ...
  for (const [person, changes] of Object.entries(effects)) {
    ...
    next[key].trust = Math.max(0, next[key].trust + (changes?.trust ?? 0));
    next[key].attraction = Math.max(
      0,
      next[key].attraction + (changes?.attraction ?? 0),
    );
    next[key].respect = Math.max(
      0,
      (next[key].respect ?? 0) + (changes?.respect ?? 0),
    );
    next[key].friction = Math.max(
      0,
      (next[key].friction ?? 0) + (changes?.friction ?? 0),
    );
    if (changes?.intent) next[key].intent = changes.intent;
  }

  const newlyCommitted = Object.entries(effects).find(
    ([, changes]) => changes?.intent === 'committed',
  )?.[0] as RelationshipKey | undefined;
  if (newlyCommitted) {
    for (const person of Object.keys(next) as RelationshipKey[]) {
      if (person === newlyCommitted) continue;
      if (
        current[person].intent === 'exploring' ||
        current[person].intent === 'committed'
      ) {
        next[person].intent = 'ended';
        next[person].trust = Math.max(0, next[person].trust - 1);
        next[person].friction += 2;
      }
    }
  }
  return next;
...
export function relationshipSummary(score: RelationshipScore) {
  const trust =
    score.trust >= 6
      ...
      : score.trust >= 3
        ...
        : 'guarded trust';
  const respect =
    score.respect >= 4 ? 'strong respect' : 'respect still forming';
  const tension =
    score.friction >= 3
      ? ', serious tension'
      : score.friction > 0
  ...
  if (score.intent === 'committed')
    return `Committed, ${trust}, ${respect}${tension}`;
  if (score.intent === 'exploring')
    return `Mutual desire, exploring what follows, ${trust}${tension}`;
  if (score.intent === 'interested')
  ...
  if (score.attraction >= 2) return `Guarded interest, ${trust}${tension}`;
  return `No romantic direction chosen, ${trust}${tension}`;
}
...
const originalNodes: Record<string, StoryNode> = {
    ...
    choices: [
      {
        ...
        addFlags: [],
        result:
          'You move one feverish guard to the rear wagon. Joren takes his place, while twelve year old Nilo carries spare bandages to the wagon.',
      },
      {
        id: 'check-horses',
        label: 'Check the horses and harness yourself.',
        detail:
          'Spend 1 Health and gain 1 Resolve. Repair the harness before it can fail on the road.',
        next: 'mara-returns',
        changes: { health: -1, resolve: 1 },
        addFlags: ['checked-horses'],
        result:
          'You replace a loose front harness strap and clean strange oil from the lead mare’s bit. The smell is too sharp to be harness grease.',
      },
      {
        id: 'check-route',
        ...
        result:
          'The low road should remain open until dusk, but the eastern clouds are moving against the wind.',
      },
      {
        id: 'check-treaty',
        label: 'Inspect the treaty wagon and its seals.',
        detail:
          'Treat the cargo as a possible target before meeting the ambassador.',
        next: 'mara-returns',
        changes: { command: 1 },
        addFlags: ['checked-treaty'],
        result:
          'The locks are intact. One wheel, however, carries a fresh scrape beside its iron pin.',
      },
    ],
  },
    ...
    choices: [
      {
        ...
        result:
          'Mara’s gaze travels over your mail and returns to your eyes. “The horse is spoken for.”',
      },
      {
        id: 'ask-marker',
        ...
        addFlags: [],
        result:
          'She draws the disturbed stones in rainwater. The marks suggest one person did the work without a cart.',
      },
      {
        id: 'admit-unease',
        ...
        addFlags: [],
        result:
          'Her teasing fades. “Then I will trust the part of you that noticed.”',
      },
    ],
  },
    ...
    choices: [
      {
        ...
        result:
          'Tivik moves the lead and replaces the scored pin. “Now it breaks only if someone truly earns it.”',
      },
      {
        id: 'lighten-wagon',
        label: 'Remove one lead chest and assign it to the guard cart.',
        detail:
          'Keep the departure time, but divide what your people must protect.',
        next: 'envoy-arrives',
        changes: { resolve: 1 },
        addFlags: ['split-cargo'],
        result:
          'The axle rises. So does the number of targets your formation must cover.',
      },
      {
        id: 'inspect-pin',
        ...
        result:
          'Under the wagon’s weight, Tivik finds three deliberate file cuts. Someone wanted the pin to fail after departure.',
      },
    ],
  },
  ...
  'envoy-arrives': {
    ...
    art: 'departure',
    body: (state) => [
      'A hand in dark green silk catches the wagon rail while Tivik is still beneath it. Ambassador Lysara Quill steps around the wheelwright instead of waiting for a formal welcome. She is taller than you expected, with silver brown hair tied behind her neck and a living coat that folds like leaves around each movement. The cloth tightens with her pulse and curls away from the cold rain.',
      'Lysara opens the wagon door herself. Inside rests a chest made from pale living wood. “If this fails to reach Bellweather, both border armies will call the silence a refusal,” she says. “By winter, they may stop waiting for peace.”',
      ...
      state.flags.includes('found-file-mark')
        ? 'Her attention settles first on Tivik, then the wheel, then you. You show her the scored pin. She does not look surprised. Your hand settles near your sword before you make it return to your side.'
      ...
      'The welcome you prepared for a sheltered diplomat dies unused. Before you can offer it, Lysara has checked Tivik’s work, the chest seal, and the road beyond the gate.',
    ...
    choices: [
      {
        ...
        addFlags: [],
        result:
          'Lysara studies your face as if weighing the exact size of your promise. Then she nods.',
      },
      {
        id: 'ask-enemies',
        ...
        addFlags: [],
        result:
          'She names three border lords and one royal minister. None should know today’s route.',
      },
      {
        id: 'answer-lysara',
        ...
        addFlags: [],
        result:
          'One corner of her mouth lifts. “Then give me a road worth judging,” Lysara says.',
      },
    ],
  },
  ...
  'sealed-case': {
    ...
    body: () => [
      ...
      'Your thumb traces the final line. You can still see your hand writing ridge road after midnight. The memory is as plain as the page denying it.',
    ...
    choices: [
      {
        ...
        result:
          'No one laughs. Brann’s hand closes around his sword belt while the gate continues to open.',
      },
      {
        id: 'trust-mara',
        ...
        result:
          'She finds your ink, your paper, and one grain of pale salt pressed beneath the wax.',
      },
      {
        id: 'test-case',
        ...
        result:
          'The hinges, lock, wax, and leather are untouched. The impossible answer survives every ordinary test.',
      },
    ],
  },
  ...
  'choose-road': {
    ...
    body: (state) => [
      state.flags.includes('declared-change')
        ? 'Nobody will meet your eyes for long. The guards heard you accuse your own sealed order, but every one of them remains beside the treaty wagon.'
      ...
      'Brann holds the reins without mounting. Mara watches the ridge. Lysara keeps one palm on the treaty chest. All three leave the last step to you.',
    ...
    choices: [
      {
        ...
        detail:
          'Follow the altered order, but deny the enemy an easy surprise.',
        next: 'low-road',
        changes: { resolve: 1 },
        addFlags: ['low-route'],
        result:
          'You place archers beside the wagon and lead the escort down toward the flooded fields.',
      },
      {
        id: 'take-ridge',
        label: 'Trust Mara and take the ridge.',
        detail:
          'Choose the route your scout saw, knowing the sky is turning dangerous.',
        next: 'ridge-road',
        addFlags: ['ridge-route'],
        result:
          'Mara rides first. The line turns uphill while thunder walks behind the eastern peaks.',
      },
      {
        id: 'delay-inspect',
        ...
        addFlags: [],
        result:
          'You close the gate yard and search under every seat, chest, cloak, and tongue.',
      },
    ],
  },
    ...
    choices: [
      {
        ...
        detail: 'Spend 2 Health pulling three people through the flood.',
        next: 'march-order',
        changes: { health: -2 },
        addFlags: ['saved-family'],
        result:
          'Cold water reaches your chest, but the farmer and both children reach the road alive.',
      },
      {
        id: 'push-low',
        ...
        result:
          'The guards struggle with the pace but obey. The bridge comes into view before the river covers its first stones.',
      },
      {
        id: 'read-flood',
        label: 'Climb the mile marker and study the flood before moving.',
        detail:
          'Spend 1 Resolve to avoid the deepest ground and expose yourself above the road.',
        next: 'march-order',
        changes: { resolve: -1, command: 1 },
        addFlags: ['read-water'],
        result:
          'From the mile marker, you see a safe curve through the field and a brief red flash on the ridge.',
      },
    ],
  },
  ...
  'ridge-road': {
    ...
    body: (state) => [
      ...
      'There is nowhere to place a shield against the whole sky. Your shoulders tighten each time the mirror flashes over the exposed wagons.',
    ...
    choices: [
      {
        ...
        detail:
          'Spend 2 Health to silence the watcher before the storm arrives.',
        next: 'march-order',
        changes: { health: -2 },
        addFlags: ['took-tower'],
        result:
          'You find a warm mirror, fresh boot prints, and a view of the escort exposed below.',
      },
      {
        id: 'shield-wagons',
        label: 'Keep the column tight and pass beneath the tower.',
        detail:
          'Spend 1 Command to protect the mission instead of chasing a watcher.',
        next: 'march-order',
        changes: { command: -1 },
        requires: { command: 1 },
        addFlags: ['tight-column'],
        result:
          'Shields rise over the wagon. The unseen watcher lets you pass without revealing a face.',
      },
      {
        id: 'false-signal',
        label: 'Use your polished shield to answer the signal.',
        detail:
          'Spend 1 Resolve and gain 1 Command. Risk being seen to learn whether someone waits ahead.',
        next: 'march-order',
        changes: { resolve: -1, command: 1 },
        addFlags: ['false-signal'],
        result:
          'A second red light answers from the valley beyond the ridge. Two groups are coordinating around you.',
      },
    ],
  },
  ...
  'inspection-yard': {
    ...
    body: (state) => [
      ...
      'Your hand closes on the stolen cape. Its clasp bears the scratch Joren made while learning to polish brass. Someone studied your Wardens closely enough to wear their trust.',
      'Brann pushes himself upright and points between the loose brake, the fleeing man, and the eastern riders. “Wagon, prisoner, or line?”',
    ],
    choices: [
      {
        id: 'catch-saboteur',
        ...
        next: 'march-order',
        changes: { health: -2 },
        addFlags: ['captured-attacker', 'rough-formation'],
        result:
          'You catch him at the gate chain. Behind you, Brann forms the line one squad too slowly.',
      },
      {
        id: 'secure-wagon',
        label: 'Secure every brake and wheel before moving.',
        detail:
          'Protect the treaty while the unknown man escapes into Greyhaven.',
        next: 'march-order',
        changes: { command: 1 },
        addFlags: ['steady-axle'],
        result:
          'Tivik clears the last wedge. The saboteur slips under the falling iron gate and disappears into Greyhaven.',
      },
      {
        id: 'form-road-line',
        ...
        result:
          'Your shields face east before the first rider crosses the far hill.',
      },
    ],
  },
    ...
    choices: [
      {
        ...
        result:
          'You take the narrow space beside the wheel where an arrow must pass through you first.',
      },
      {
        id: 'command-speed',
        ...
        result:
          'The line lengthens its stride. Tired people save minutes now and may owe them later.',
      },
      {
        id: 'send-mara-ahead',
        label: 'Send Mara ahead to find the attackers.',
        detail:
          'Trust her alone beyond your protection and gain warning if she returns.',
        next: 'road-conversation',
        changes: { command: 1 },
        addFlags: ['mara-ahead'],
        result:
          'Mara touches two fingers to your wrist, then disappears between the wet trees.',
      },
      {
        ...
        result:
          'Heat wakes inside your chest. The magic accepts your promise, and you can suddenly feel every life you must protect.',
      },
    ],
  },
  ...
  'road-conversation': {
    ...
    body: (state) => [
      ...
      state.flags.includes('mara-ahead')
        ? 'Mara appears between two alder trees and points to fresh tracks ahead. She waits beside your stirrup. “One minute before I ride back. Use it.”'
        : 'Mara’s knee touches yours again as she turns to watch the rear of the column. “Whatever you decide about the seed, decide before the next bend.”',
    ],
    choices: [
      {
        ...
        detail:
          'Protect the person while accepting that she withheld a danger.',
        next: 'ambush-warning',
        addFlags: [],
        result:
          'Lysara closes your fingers around the cool seed, then takes it back before your hands part.',
      },
      {
        id: 'tell-brann-seed',
        ...
        addFlags: [],
        result:
          'Brann changes the formation without asking questions. Lysara’s face becomes carefully unreadable.',
      },
      {
        id: 'ask-mara-future',
        label: 'Ask Mara what she planned for the evening.',
        detail:
          'Available with established Mara trust. Take one personal moment while the road is briefly quiet.',
        next: 'ambush-warning',
        changes: { resolve: 1 },
        requiresRelationships: { mara: { trust: 3 } },
        addFlags: ['planned-evening'],
        result:
          '“A bath, a fire, and deciding whether you look better without the cloak,” she says. Then her eyes snap toward the trees.',
      },
      {
        id: 'watch-tree-line',
    ...
    body: (state) => [
      state.flags.includes('mara-ahead')
        ...
        : state.flags.includes('watched-trees')
          ? 'Your warning reaches Brann while the hidden bow is still bending. The first black arrow strikes a shield instead of a throat.'
      ...
      state.flags.includes('oath-safe-arrival')
        ? 'Oathfire climbs your ribs like a second heartbeat. Four endangered lives pull at once, each in a different direction.'
        : 'Brann waits for your order while blood runs between his fingers around the trapped guard’s shoulder.',
      'Brann slams his shield over the trapped guard. “Order, Captain!”',
    ],
    choices: [
      {
        id: 'guard-opening',
        ...
        result:
          'Two arrows strike your shield. The third cuts your thigh, shallow but hot.',
      },
      {
        id: 'command-opening',
        label: 'Turn the escort into two moving walls.',
        detail:
          'Spend 2 Command to keep guards, wagon, and horses acting together.',
        next: routeDestination,
        changes: { command: -2 },
        requires: { command: 2 },
        addFlags: ['ordered-walls'],
        result:
          'Your voice reaches people before fear does. Shields lock and the wagon keeps rolling.',
      },
      {
        id: 'trust-mara-opening',
        label: 'Follow Mara’s signal without looking for her.',
        detail:
          'Available with established Mara trust. Trust her view of the trap over your own.',
        next: routeDestination,
        requiresRelationships: { mara: { trust: 3 } },
        addFlags: [],
        result:
          'You move right as her arrow crosses left. A hidden attacker falls into the space you refused to occupy.',
      },
      {
        id: 'spend-oathfire-opening',
        label: 'Call on the promise holding every life to you.',
        detail:
          'Spend 1 Oathfire to feel where the next mortal blow will land.',
        next: routeDestination,
        changes: { oathfire: -1 },
        requires: { oathfire: 1 },
        addFlags: ['felt-mortal-blow'],
        result:
          'For one fierce instant, every endangered heartbeat pulls a bright thread through your chest.',
      },
    ],
  },
    ...
    choices: [
      {
        ...
        detail:
          'Spend 1 Health. Protect lives while treaty pages and the attacker may escape.',
        next: 'aftermath',
        changes: { health: -1 },
        hideIfAnyFlags: ['saved-family'],
        addFlags: ['treaty-damaged'],
        result:
          'You drag Joren and the second guard onto solid stone while pale treaty pages scatter into brown water.',
      },
      {
        id: 'save-people-low-family',
        label: 'Leave the chest and pull everyone clear of the flood.',
        detail:
          'Spend 1 Health. Protect Joren and both children while treaty pages and the attacker may escape.',
        advantage:
          'Your rescue should bring Joren and both children onto solid stone.',
        next: 'aftermath',
        changes: { health: -1 },
        showIfAnyFlags: ['saved-family'],
        addFlags: ['treaty-damaged'],
        result:
          'You drag Joren and both children onto solid stone while pale treaty pages scatter into brown water.',
      },
      {
        id: 'save-treaty-low',
        label: 'Hold the wagon while Brann clears the broken span.',
        detail:
          'Spend 2 Health. Protect the treaty while trusting Brann with the trapped people.',
        next: 'aftermath',
        changes: { health: -2, command: 1 },
        requires: { health: 2 },
        hideIfAnyFlags: ['saved-family'],
        addFlags: ['treaty-safe', 'guard-wounded'],
        result:
          'Your boots slide to the bridge edge. Brann pulls Joren and the second guard clear before your strength gives out.',
      },
      {
        id: 'save-treaty-low-family',
        label: 'Hold the wagon while Brann clears the broken span.',
        detail:
          'Spend 2 Health. Protect the treaty while trusting Brann to pull Joren and both children clear.',
        advantage:
          'Brann should save Joren and both children while you keep the treaty intact.',
        next: 'aftermath',
        changes: { health: -2, command: 1 },
        requires: { health: 2 },
        showIfAnyFlags: ['saved-family'],
        addFlags: ['treaty-safe', 'guard-wounded'],
        result:
          'Your boots slide to the bridge edge. Brann pulls Joren and both children clear before your strength gives out.',
      },
      {
        id: 'capture-low',
        label: 'Order Mara to take the runner alive.',
        detail:
          'Spend 1 Command. Spend 1 Resolve. Mara pursues the runner while Brann pulls Joren and the second guard clear.',
        advantage:
          'The divided order should capture an attacker without abandoning the people beside the broken span.',
        next: 'aftermath',
        changes: { command: -1, resolve: -1 },
        requires: { command: 1 },
        hideIfAnyFlags: ['saved-family'],
        ...
        result:
          'Mara brings the runner down. Brann pulls Joren and the second guard clear, but the guard’s leg breaks beneath the wagon. The treaty chest strikes stone and cracks.',
      },
      {
        id: 'capture-low-family',
        label: 'Order Mara to take the runner alive.',
        detail:
          'Spend 1 Command. Spend 1 Resolve. Mara pursues the runner while Brann pulls Joren and both children clear.',
        advantage:
          'The divided order should capture an attacker without abandoning the rescued children beside the broken span.',
        next: 'aftermath',
        changes: { command: -1, resolve: -1 },
        requires: { command: 1 },
        showIfAnyFlags: ['saved-family'],
        addFlags: ['captured-attacker', 'treaty-damaged', 'guard-wounded'],
        result:
          'Mara brings the runner down. Brann pulls Joren and both children clear, but a guard’s leg breaks beneath the wagon. The treaty chest strikes stone and cracks.',
      },
    ],
  },
    ...
    choices: [
      {
        ...
        changes: { health: -2 },
        requires: { health: 2 },
        addFlags: ['saved-mara', 'treaty-damaged'],
        result:
          'You lock one arm around the root and haul both of them up as the wagon breaks behind you.',
      },
      {
        id: 'hold-wagon-ridge',
        label: 'Brace the wagon and order Brann to the ledge.',
        detail:
          'Spend 2 Health. Protect the treaty while trusting Brann with Mara.',
        next: 'aftermath',
        changes: { health: -2, command: 1 },
        requires: { health: 2 },
        addFlags: ['treaty-safe', 'mara-hurt'],
        result:
          'Your shoulder nearly leaves its socket. Brann reaches Mara, but the root tears her palm open before he does.',
      },
      {
        id: 'pursue-signaler',
        ...
        result:
          'You catch him among the wooden frames. Below, the wagon hits stone and Mara screams once.',
      },
    ],
  },
    ...
    choices: [
      {
        ...
        changes: { health: -2, resolve: 1 },
        requires: { health: 2 },
        addFlags: ['treaty-safe'],
        result:
          'Your shield takes a sword edge. The riders scatter, leaving blood and three empty saddles.',
      },
      {
        id: 'protect-lysara',
        ...
        changes: { command: -1 },
        requires: { command: 1 },
        addFlags: ['treaty-damaged', 'guard-wounded'],
        result:
          'Lysara survives inside a wall of shields. Axes open the wagon before the riders retreat.',
      },
      {
        id: 'take-leader',
        label: 'Trust Mara with the escort and take the leader alive.',
        detail:
          'Available when Mara trusts you with command. Spend 1 Resolve to capture the person carrying answers.',
        next: 'aftermath',
        changes: { resolve: -1 },
        requiresRelationships: { mara: { trust: 4 } },
        addFlags: ['captured-attacker', 'treaty-damaged', 'guard-wounded'],
        result:
          'Mara takes command without hesitation. You drag the leader from his saddle and keep him breathing.',
      },
      {
        id: 'oath-line',
        ...
        addFlags: ['treaty-damaged'],
        result:
          'Your shield grows hot. For ten heartbeats, no rider can cross the line you named.',
      },
    ],
  },
  ...
  aftermath: {
    ...
    body: (state) => [
      ...
      'Joren’s bandage darkens. A branch breaks beyond the nearest shield. Mara reaches for the cut at your thigh and waits for you to stop her or let her work.',
    ],
    choices: [
      {
        id: 'treat-wounded',
        ...
        result:
          'You hold Joren still while Lysara closes the wound with sap that smells of pine smoke.',
      },
      {
        id: 'secure-ground',
        label: 'Set shields, watchers, and a retreat order.',
        detail:
          'Spend 1 Command. Make the road defensible before investigating it.',
        next: 'evidence',
        changes: { command: -1, resolve: 1 },
        requires: { command: 1 },
        addFlags: ['secure-perimeter'],
        result:
          'Brann builds a hard little island of shields, wagons, and people who know where to run.',
      },
      {
        id: 'steady-mara',
        label: 'Check Mara’s wounds and let her check yours.',
        detail:
          'Available with established Mara trust. Recover 1 Resolve while the guards form a defensive ring.',
        next: 'evidence',
        changes: { resolve: 1 },
        requiresRelationships: { mara: { trust: 3 } },
        addFlags: ['mara-tended'],
        result:
          'Her fingers are careful at your thigh. “You are allowed to bleed,” she says. “You are not allowed to hide it.”',
      },
    ],
  },
    ...
    body: (state) => [
      ...
      state.flags.includes('secure-perimeter')
        ...
        : state.flags.includes('wounded-stable')
          ...
          : state.flags.includes('mara-tended')
            ? 'While Mara knots the bandage at your thigh, Brann finishes the defensive ring. The wounded remain restless, but no one must leave cover to watch the trees.'
    ...
    choices: [
      {
        ...
        result:
          'He received all three possible ambush plans before dawn, each bearing your signature.',
      },
      {
        id: 'open-wax-tube',
        ...
        addFlags: ['confirmed-advance-orders'],
        result:
          'Inside are three route cards. Every one was sealed before your dispatch case left your room.',
      },
      {
        id: 'test-salt',
        label: 'Have Lysara test the salt while Mara opens the wax tube.',
        detail:
          'Confirm the prepared routes and learn why the distant coast matters.',
        next: 'retreat',
        addFlags: ['confirmed-advance-orders'],
        result:
          'Lysara identifies fresh Shard Coast salt while Mara opens the tube beside you. It holds three route cards sealed before your dispatch case left your room.',
      },
    ],
  },
  ...
  retreat: {
    ...
    body: (state) => [
      ...
      'You want walls, closed gates, and one more count of the living. Then the wet departure ledger returns to you with two old names beneath canvas. Before turning toward safety, you count how many miles it will cost.',
      ...
      'A black arrow strikes the mile stone behind Brann. No archer shows between the trees.',
    ],
    choices: [
      {
        id: 'turn-west',
        ...
        detail:
          'Seek walls and reinforcements, even if the storm gate is closed.',
        next: 'folded-road',
        changes: { command: -1 },
        addFlags: ['turned-west'],
        result:
          'The escort turns. Every wounded step now leads toward the city that should be behind you.',
      },
      {
        id: 'press-bellweather',
        label: 'Press east toward Bellweather Inn.',
        detail:
          'Spend 1 Health and gain 1 Resolve. Pursue shelter while attackers remain ahead.',
        next: 'folded-road',
        changes: { health: -1, resolve: 1 },
        addFlags: ['pressed-east'],
        result:
          'You put the strongest walkers around the stretchers and follow the road into hard rain.',
      },
      {
        id: 'climb-scout',
        ...
        result:
          'You climb through thorn and rain until the road opens beneath you like a grey ribbon.',
      },
    ],
  },
  ...
  'folded-road': {
    ...
    body: (state) => [
      ...
      state.flags.includes('turned-west')
        ...
        : state.flags.includes('pressed-east')
          ...
          : 'Your compass still points east, but Greyhaven stands where Bellweather should be.',
      'Lysara raises her glass seed toward the drowned road. Green light reaches for the water, and the seed cracks in her palm. One living shard cuts through her glove before she can close her hand.',
    ...
    choices: [
      ...
      },
      {
        ...
        result:
          'You turn away from the impossible shore long enough to find a hill the wounded can defend.',
      },
      {
        id: 'swear-home-oath',
        ...
        result:
          'Oath magic burns through your body. For one heartbeat, the road beneath the sea answers your promise.',
      },
    ],
  },
  ...
  'ending-height': {
    ...
    body: () => [
      ...
      'From above, the sea forms a perfect bend instead of a natural shore. You trace it with one finger. Tivik folds the wet route map until Greyhaven and Bellweather touch. The water below follows the same crease.',
      'Now you can see what happened: distant parts of the road have been pushed together. The view offers no clue who had enough power to do it.',
      'Pale salt glitters along the new tide line, fresh as frost.',
      'A lantern moves inside Bellweather Inn on the far side. Then a second lantern answers from the woods behind your hill. Your hand settles on the sword between them.',
    ],
    choices: [],
  },
  ...
  'ending-oath': {
    ...
    nextChapter: 'c2-arrival',
    body: (state) => [
      'Red light settles between the rings of your armour. Mara sees it and steps close, but the heat belongs to every survivor your new promise has named.',
      state.flags.includes('oath-safe-arrival')
      ...
      'Across the water, Bellweather Inn lights one window. A figure raises one hand behind the glass. A second shape moves behind them, and the light goes out.',
    ],
    choices: [],
  },
  ...
  'c2-arrival': {
    ...
    location: 'The Folded King’s Road',
    objective:
      'Reach Bellweather Inn before something follows you from the water.',
    threat: 'Immediate',
    art: 'inn',
    lesson: {
    ...
    choices: [
      {
        ...
        result:
          'The guards form a tight wall. The creature follows to the edge of the yard, then sinks out of sight.',
      },
      {
        id: 'c2-lead-water',
        ...
        result:
          'You keep your shield toward the water until the last horse reaches mud. The pale eye watches only you.',
      },
      {
        id: 'c2-trust-mara-entry',
        label: 'Let Mara choose the safest path through the water.',
        detail:
          'Available when Mara trusts you with command. Trust her judgment while you watch the wounded.',
        next: 'c2-threshold',
        requiresRelationships: { mara: { trust: 4 } },
        addFlags: ['c2-mara-led-entry'],
        result:
          'Mara finds firm road stones beneath the water. Nobody falls, and the creature never gets close enough to strike.',
      },
      {
        id: 'c2-oath-guided-entry',
        label: 'Use your Oath to feel which survivor is in danger.',
        detail:
          'Spend 1 Oathfire. Find the weakest point in the group before the creature does.',
        next: 'c2-threshold',
        changes: { oathfire: -1 },
        requires: { oathfire: 1 },
        addFlags: ['c2-oath-found-child'],
        result:
          'One life burns colder than the rest. A boy hidden in the wagon is losing blood beneath a wool blanket.',
      },
      {
        id: 'c2-guard-rear',
        label: 'Guard the rear while Brann leads everyone to the inn.',
        detail:
          'Save your remaining resources but give the creature time to study you.',
        next: 'c2-threshold',
        addFlags: ['c2-creature-studied-caelan'],
        result:
          'The creature keeps its distance. Just before it sinks, its pale eye changes until it looks like your own.',
      },
    ],
  },
    ...
    choices: [
      {
        ...
        result:
          'The common room is empty. The back door opens onto the same yard as the front door.',
      },
      {
        id: 'c2-carry-first',
        ...
        result:
          'Maelin clears a table before you reach it. She knows exactly where you are going to set him down.',
      },
      {
        id: 'c2-question-name',
        label: 'Ask Maelin how she knows your names.',
        detail:
          'Keep everyone in the defended yard until she gives a clear answer.',
        next: 'c2-triage',
        changes: { resolve: 1 },
        addFlags: ['c2-demanded-answer'],
        result:
          'Maelin grips the lamp. “You told me yesterday. I have been waiting eleven years for yesterday to happen again.”',
      },
    ],
  },
  ...
  'c2-triage': {
    ...
    objective:
      'Stabilise the wounded before the creature or the attackers return.',
    threat: 'Rising',
    art: 'inn',
    body: (state) => [
    ...
    choices: [
      {
        ...
        detail:
          'Spend 1 Command. Reduce panic before choosing who gets the medicine.',
        next: 'c2-medicine',
        changes: { command: -1, resolve: 1 },
        requires: { command: 1 },
        addFlags: ['c2-organised-care'],
        result:
          'Mara presses cloth to Nilo’s leg. Lysara explains the seed wound. Brann keeps the attacker breathing.',
      },
      {
        id: 'c2-work-beside-mara',
        label: 'Work beside Mara and stop Nilo’s bleeding.',
        detail:
          'Spend 1 Health. Gain trust and a clearer view of the child’s injury.',
        next: 'c2-medicine',
        changes: { health: -1 },
        requires: { health: 1 },
        addFlags: ['c2-compressed-wound'],
        result:
          'You hold pressure while Mara ties the bandage. Nilo stays awake, but the bleeding does not fully stop.',
      },
      {
        ...
        result:
          'Lysara gives calm instructions while green light spreads across her wrist. She understands every wound except her own.',
      },
    ],
  },
    ...
    choices: [
      {
        ...
        detail:
          'Spend 1 Medicine. Save her hand and protect the second proof of peace.',
        next: 'c2-eleven-years',
        changes: { medicine: -1 },
        requires: { medicine: 1 },
        addFlags: ['c2-saved-lysara'],
        result:
          'The green lines withdraw from Lysara’s arm. She closes her fingers around yours and says, “I know what this cost.”',
      },
      {
        id: 'c2-medicine-nilo',
        label: 'Give the medicine to Nilo.',
        detail:
          'Spend 1 Medicine. Stop the child’s bleeding before it is too late.',
        next: 'c2-eleven-years',
        changes: { medicine: -1, resolve: 1 },
        requires: { medicine: 1 },
        addFlags: ['c2-saved-nilo'],
        result:
          'Warmth returns to Nilo’s face. Maelin sits beside him and presses both hands over her mouth.',
      },
      {
        id: 'c2-medicine-attacker',
        label: 'Give the medicine to the attacker.',
        detail:
          'Spend 1 Medicine. Keep a dangerous witness alive long enough to answer questions.',
        next: 'c2-eleven-years',
        changes: { medicine: -1, command: 1 },
        requires: { medicine: 1 },
        addFlags: ['c2-saved-attacker'],
        result:
          'The attacker’s breathing steadies. Brann looks angry, but he tightens the prisoner’s bandage exactly as you ordered.',
      },
    ],
  },
    ...
    choices: [
      {
        ...
        result:
          'Her shoulders lower. “Good. I am tired of proving I was lonely.”',
      },
      {
        ...
        result:
          'The escort remembers passing an empty field. Maelin remembers waving from the door. Both memories include the same lightning strike.',
      },
      {
        id: 'c2-test-book',
        label: 'Check the guest book, ink, and paper for a trick.',
        detail:
          'Look for ordinary proof before accepting an impossible answer.',
        next: 'c2-investigate',
        changes: { resolve: 1 },
        addFlags: ['c2-tested-ledger'],
        result:
          'The ink ages from fresh black to faded brown across eleven years. The book is real.',
      },
    ],
  },
    ...
    choices: [
      {
        ...
        result:
          'Maelin turns to the first page bearing your name. A line beneath it has been cut out with a knife.',
      },
      {
        id: 'c2-cellar-route',
        label: 'Inspect the shaking cellar with Tivik.',
        detail:
          'Spend 1 Health. Search the most dangerous part of the building first.',
        next: 'c2-cellar',
        changes: { health: -1 },
        requires: { health: 1 },
        addFlags: ['c2-cellar-route'],
        result:
          'The cellar steps tilt while you descend, then settle pointing in a different direction.',
      },
      {
        id: 'c2-cellar-route-prepared',
        label: 'Follow the scratches you found to the cellar with Tivik.',
        detail:
          'Use the entrance search you already completed to reach the hidden break safely.',
        advantage:
          'The earlier search should reveal the cellar danger without another resource cost.',
        next: 'c2-cellar',
      ...
      },
      {
        ...
        result:
          'The attacker opens his eyes when you place the sealed order case beside his face.',
      },
    ],
  },
    ...
    choices: [
      {
        ...
        next: 'c2-night-watch',
        changes: { command: 1 },
        addFlags: ['c2-knows-midnight-pattern'],
        result:
          'You mark midnight, the cellar, and the words road pin. The useful pattern fits on one page.',
      },
      {
        id: 'c2-ask-missing-mara',
        label: 'Ask Maelin what happened when Mara was missing.',
        detail:
          'Risk a painful answer to learn what the changing road can alter.',
        next: 'c2-night-watch',
        changes: { resolve: -1 },
        addFlags: ['c2-heard-mara-version'],
        result:
          '“You searched for her until the bell rang,” Maelin says. “Then the road returned you without her.”',
      },
      {
        id: 'c2-study-cloak-version',
        label: 'Study the entry about the man wearing your cloak.',
        detail:
          'Follow the clue that resembles the figure seen across the water.',
        next: 'c2-night-watch',
        changes: { resolve: 1 },
        addFlags: ['c2-cloak-double'],
        result:
          'The man knew your name but had a deep burn across his face. Maelin never saw him remove the cloak.',
      },
    ],
  },
    ...
    choices: [
      {
        ...
        next: 'c2-night-watch',
        changes: { command: 1 },
        addFlags: ['c2-tracked-stone'],
        result:
          'The marked stone moves toward the front door, then appears beside the back door without crossing the room.',
      },
      {
        id: 'c2-break-bracket',
        label: 'Pull the iron spike from its bracket.',
        detail:
          'Spend 1 Health. Take a possible tool before the unseen footsteps reach the stairs.',
        next: 'c2-night-watch',
        changes: { health: -1 },
        requires: { health: 1 },
        addFlags: ['c2-has-pin-key'],
        result:
          'The bracket breaks. The spike is heavy, warm, and shaped to fit something much larger.',
      },
      {
        id: 'c2-follow-footsteps',
        label: 'Follow the footsteps from below.',
        detail:
          'Risk the moving cellar to learn who is walking in the empty rooms.',
        next: 'c2-night-watch',
        changes: { resolve: -1 },
        addFlags: ['c2-tracked-double'],
        result:
          'The footsteps stop over your head. A man with your voice says, “Not this door. Not yet.”',
      },
    ],
  },
    ...
    choices: [
      {
        ...
        detail:
          'Spend 1 Resolve. Push for a direct answer before his condition changes.',
        next: 'c2-night-watch',
        changes: { resolve: -1, command: 1 },
        addFlags: ['c2-crown-voice'],
        result:
          'Garran never saw a face. He heard a calm man speaking from behind a royal screen in Greyhaven Palace.',
      },
      {
        id: 'c2-offer-protection',
        label: 'Offer Garran protection if he testifies.',
        detail:
          'Make a normal promise, not a magical Oath. His answer will depend on whether he trusts you.',
        next: 'c2-night-watch',
        addFlags: ['c2-offered-sable-safety'],
        result:
          'Garran studies you, then gives one warning. “The Crown has people inside your Wardens.”',
      },
      {
        id: 'c2-take-orders',
        label: 'Take the orders and let him rest.',
        detail:
          'Preserve physical evidence instead of risking his life for another answer.',
        next: 'c2-night-watch',
        changes: { resolve: 1 },
        addFlags: ['c2-kept-crown-orders'],
        result:
          'You place the sealed orders inside your armour. Garran closes his eyes but keeps breathing.',
      },
    ],
  },
    ...
    choices: [
      {
        ...
        result:
          'Mara takes your hand. “Then stop carrying us as one weight. We are people. Let us carry you back.”',
      },
      {
        id: 'c2-kiss-mara',
        label: 'Touch her hand and ask if you may kiss her.',
        detail:
          'Available when Mara shares enough trust and attraction. Let desire become a clear choice for both of you.',
        next: 'c2-bell',
        changes: { resolve: 1 },
        requiresRelationships: { mara: { trust: 4, attraction: 3 } },
        addFlags: [],
        result:
          'Mara says yes and closes the last space herself. Her fingers settle against your jaw. Her mouth is warm, slow at first, then certain enough to make the cold landing disappear. The road changes outside, and she parts from you with her breath still touching your lips.',
      },
      {
        id: 'c2-choose-mara-friendship',
        label:
          'Tell Mara you want her beside you as the friend who knows you best.',
        detail:
          'Choose a close friendship without leaving romance silently pending.',
        next: 'c2-bell',
        changes: { resolve: 1 },
        result:
          'Mara turns her hand and grips yours wrist to wrist, the way you did after surviving your first patrol. “That is not a lesser place beside you,” she says. “Do not treat it like one.”',
      },
      {
        ...
        result:
          'Mara nods, but leaves her hand where it is. “I asked what you fear, not what you plan to do.”',
      },
    ],
  },
    ...
    choices: [
      {
        ...
        detail:
          'Spend 1 Command. Keep everyone connected while the room changes.',
        next: 'c2-common-room-crisis',
        changes: { command: -1 },
        requires: { command: 1 },
        addFlags: ['c2-rope-line'],
        result:
          'The floor stretches, but the rope keeps every person linked to the next.',
      },
      {
        id: 'c2-hold-door',
        label: 'Brace the front door before the other group enters.',
        detail:
          'Spend 2 Health. Stop the two versions of the inn from meeting.',
        next: 'c2-common-room-crisis',
        changes: { health: -2 },
        requires: { health: 2 },
        addFlags: ['c2-held-door'],
        result:
          'The door pushes back with your own strength. Through the gap, your other face looks directly at you.',
      },
      {
        id: 'c2-oath-anchor',
        label: 'Use your Oath to hold every promised life in this room.',
        detail:
          'Spend 2 Oathfire. The Oath protects people, but not the building.',
        next: 'c2-common-room-crisis',
        changes: { oathfire: -2 },
        requires: { oathfire: 2 },
        addFlags: ['c2-oath-anchored'],
        result:
          'Warm points of life fill your awareness. Walls move through darkness, but nobody disappears.',
      },
      {
        id: 'c2-follow-maelin',
        ...
        result:
          'Maelin opens a door that was a cupboard one moment earlier. Stone steps wait behind it.',
      },
    ],
  },
    ...
    choices: [
      {
        ...
        detail:
          'Spend 2 Command. Use the same commands all versions were trained to obey.',
        next: 'c2-descend',
        changes: { command: -2 },
        requires: { command: 2 },
        addFlags: ['c2-no-fight'],
        result:
          'Your voice and the burned man’s voice collide. Enough guards hesitate for Mara to clear the cellar door.',
      },
      {
        id: 'c2-show-orders',
        ...
        result:
          'The nearest guards recognise the Crown wax. They turn their weapons away from Maelin and toward the burned man.',
      },
      {
        id: 'c2-protect-current-group',
        label: 'Form a shield line and move your own group downstairs.',
        detail:
          'Spend 1 Health. Avoid the other versions instead of defeating them.',
        next: 'c2-descend',
        changes: { health: -1 },
        requires: { health: 1 },
        addFlags: ['c2-shielded-descent'],
        result:
          'Shields strike shields, but nobody swings a blade. Your group reaches the stairs together.',
      },
      {
        id: 'c2-give-maelin-key',
        label: 'Ask Maelin to open the hidden cellar path.',
        detail:
          'Available because you trusted or believed her before the crisis.',
        next: 'c2-descend',
        requiresFlags: ['c2-trusted-maelin'],
        addFlags: ['c2-maelin-secret-path'],
        result:
          'Maelin turns her lamp hook inside a wall crack. A narrow stair opens behind the fireplace.',
      },
      {
        id: 'c2-block-versions',
        label: 'Tip the heavy tables over and divide the room.',
        detail:
          'Create a path without spending a resource, but leave supplies behind.',
        next: 'c2-descend',
        addFlags: ['c2-left-supplies'],
        result:
          'The tables slow the other group. Your people reach the cellar, but food and blankets remain on the wrong side.',
      },
    ],
  },
    ...
    choices: [
      {
        ...
        result:
          'Mara lights an arrow from the fire and follows you into the moving dark.',
      },
      {
        id: 'c2-take-lysara',
        ...
        result:
          'Lysara ties back her injured hand and follows. Mara takes your place at the stair without complaint.',
      },
      {
        id: 'c2-take-maelin',
    ...
    choices: [
      {
        ...
        result:
          'The rope bends around empty air, revealing a turn your eyes cannot see.',
      },
      {
        id: 'c2-use-pin-key',
        ...
        result:
          'The iron pulls toward the silver crack. A hidden section of road becomes solid beneath your feet.',
      },
      {
        id: 'c2-follow-companion',
        label: 'Trust your companion’s reading of the cellar.',
        detail:
          'Spend 1 Resolve. Follow a human judgment when your own senses disagree.',
        next: 'c2-road-pin',
        changes: { resolve: -1 },
        requires: { resolve: 1 },
        addFlags: ['c2-trusted-below'],
        result:
          'You stop trying to force the cellar into a normal shape. Your companion leads you across the broken distance.',
      },
      {
        id: 'c2-follow-lysara',
        label: 'Trust Lysara’s living thread to mark the true road.',
        detail:
          'Spend 1 Resolve. Follow her magic when your own eyes disagree.',
        advantage:
          'Lysara’s thread should find a safe path before the mountain ledge falls.',
        next: 'c2-road-pin',
        changes: { resolve: -1 },
        requires: { resolve: 1 },
        requiresFlags: ['c2-lysara-below'],
        showIfAllFlags: ['c2-lysara-below'],
        addFlags: ['c2-trusted-below', 'c2-trusted-lysara-path'],
        result:
      ...
      {
        id: 'c2-follow-maelin-path',
        label: 'Trust Maelin’s knowledge of the old supports.',
        detail:
          'Spend 1 Resolve. Follow the builder marks she has watched for thirty years.',
        advantage:
          'Maelin’s route should keep the group on Bellweather stone when the false road drops away.',
        next: 'c2-road-pin',
        changes: { resolve: -1 },
        requires: { resolve: 1 },
        requiresFlags: ['c2-maelin-below'],
        showIfAllFlags: ['c2-maelin-below'],
        addFlags: ['c2-trusted-below', 'c2-trusted-maelin-path'],
        result:
    ...
    choices: [
      {
        ...
        detail:
          'Learn how to move the pin without breaking the road completely.',
        next: 'c2-remove-pin',
        changes: { command: 1 },
        addFlags: ['c2-safe-removal'],
        result:
          'Tivik finds three locking teeth. Two are broken. The last can guide the pin out if everyone pulls together.',
      },
      {
        id: 'c2-read-crown-mark',
        label: 'Compare the Crown mark with the sealed orders.',
        detail:
          'Connect the sabotage to a tool controlled by Greyhaven Palace.',
        next: 'c2-remove-pin',
        changes: { resolve: 1 },
        addFlags: ['c2-proved-crown-tool'],
        result:
          'The cut and the wax seal use the same small crown with one damaged point. The evidence matches.',
      },
      {
        id: 'c2-feel-oath-pin',
        label: 'Touch the pin and ask your Oath what lives depend on it.',
        detail:
          'Spend 1 Oathfire. Feel the human cost before choosing how to remove it.',
        next: 'c2-remove-pin',
        changes: { oathfire: -1 },
        requires: { oathfire: 1 },
        addFlags: ['c2-felt-road-lives'],
        result:
          'Hundreds of travellers flare at the edge of your awareness. The broken road is threatening people far beyond the inn.',
      },
    ],
  },
    ...
    choices: [
      {
        ...
        detail:
          'Spend 2 Command. Remove the pin through coordination instead of force.',
        next: 'c2-last-testimony',
        changes: { command: -2 },
        requires: { command: 2 },
        addFlags: ['c2-pin-whole'],
        result:
          'Your count travels up the chain. The pin rises one hand at a time until it tears free in one piece.',
      },
      {
        id: 'c2-strength-pull',
        label: 'Join the chain and pull with everything left in you.',
        detail:
          'Spend 3 Health. Free the pin before the burned man can stop you.',
        next: 'c2-last-testimony',
        changes: { health: -3, resolve: 1 },
        requires: { health: 3 },
        addFlags: ['c2-pin-whole', 'c2-caelan-injured'],
        result:
          'Pain tears across your back. The pin comes free, and you fall with it across your legs.',
      },
      {
        id: 'c2-oath-pull',
        label: 'Swear that this road will carry the living home.',
        detail:
          'Spend 2 Resolve. Gain 3 Oathfire and accept a binding duty to repair the road.',
        next: 'c2-last-testimony',
        changes: { resolve: -2, oathfire: 3 },
        requires: { resolve: 3 },
        addFlags: ['c2-oath-repair-road', 'c2-pin-whole'],
        result:
          'Fire crosses the chain. Every living hand pulls at once, and the road pin rises toward your promise.',
      },
      {
        id: 'c2-wagon-break',
        label: 'Drop the treaty wagon through the silver crack.',
        detail:
          'Sacrifice the wagon and break the pin free without spending a resource.',
        next: 'c2-last-testimony',
        addFlags: ['c2-pin-broken', 'treaty-damaged'],
        result:
          'The wagon falls through the crack and yanks the chain tight. The pin snaps. Most of it comes free, but a black fragment remains below.',
      },
      {
        id: 'c2-wagon-sacrifice',
        label: 'Break the supply wagon against the pin.',
        detail:
          'Spend no stat, but lose the wagon and supplies that cannot be carried.',
        advantage:
          'The heavy wagon should seat the pin when you lack the strength or authority for another method.',
        next: 'c2-last-testimony',
  ...
  'c2-last-testimony': {
    ...
    choices: [
      ...
      },
      {
        ...
        result:
          'The promise catches fire inside your chest. The Crown mark on the iron glows in answer.',
      },
    ],
  },
  ...
  'c2-ending-testimony': {
    ...
    objective:
      'Keep Garran alive and learn who inside the Wardens served the Crown plot.',
    threat: 'Rising',
    art: 'inn',
    final: true,
  ...
  'c2-ending-pin': {
    ...
    location: 'The Eastern Road to Harrowfen',
    objective:
      'Reach Harrowfen and learn why the road pin is pulling toward it.',
    threat: 'Unknown',
    art: 'inn',
    final: true,
    nextChapter: 'c3-arrival',
  ...
  'c3-arrival': {
    ...
    objective:
      'Enter Harrowfen without letting the false escort divide your people.',
    threat: 'Immediate',
    art: 'harrowfen',
    body: (state) => [
    ...
    choices: [
      {
        ...
        result:
          'Your order moves down the line. Twenty blades return to their sheaths before one frightened hand can begin a battle.',
      },
      {
        id: 'c3-shield-wounded',
        label: 'Place yourself between the archers and the wounded.',
        detail:
          'Spend 1 Health to protect the wagons while Mara speaks for you.',
        next: 'c3-gate',
        requires: { health: 1 },
        changes: { health: -1 },
        addFlags: ['c3-shielded-wounded'],
        result:
          'You stand in the open with empty hands raised. Mara names every injured traveller behind you until the archers begin to look ashamed.',
      },
      {
        id: 'c3-show-evidence',
        label: 'Hold up the Crown marked iron.',
        detail:
          'Make the guards look at the strange evidence instead of your weapons.',
        next: 'c3-gate',
        addFlags: ['c3-showed-iron'],
        result:
          'The iron turns in your grip and points through the closed gate. Several guards step back as if it has named them.',
      },
    ],
  },
    ...
    choices: [
      {
        ...
        result:
          'The fresh signature matches the old one until Elene holds both near the iron. The older ink crawls toward it.',
      },
      {
        id: 'c3-give-command-word',
        label: 'Ask Brann for yesterday’s command word.',
        detail:
          'Use a Warden check that an imitation should not know. Requires 2 Command.',
        next: 'c3-triage',
        requires: { command: 2 },
        changes: { command: -1 },
        addFlags: ['c3-proved-command'],
        result:
          'Brann gives the challenge. You answer with the private reply. Elene admits the earlier Caelan knew a word that will not be chosen until tomorrow.',
      },
      {
        id: 'c3-let-mara-search-you',
        label: 'Let Mara hand Elene your weapons and papers.',
        detail:
          'Accept a temporary loss of control and let Mara handle your weapons.',
        next: 'c3-triage',
        addFlags: ['c3-entered-unarmed'],
        result:
          'Mara removes your sword with careful hands. “I expect this back,” she tells Elene. The warning earns the first small smile on the wall.',
      },
    ],
  },
  ...
  'c3-triage': {
    ...
    objective:
      'Choose what the escort needs before the false arrival strikes again.',
    threat: 'Rising',
    art: 'harrowfen',
    lesson: {
    ...
    choices: [
      {
        ...
        result:
          'You give Mara command of the wagons and follow Lysara toward the old watch house.',
      },
      {
        id: 'c3-focus-wounded',
        label: 'Secure the healer with Mara.',
        detail:
          'Protect the people already paying for your mission and support Mara’s priority.',
        next: 'c3-healer',
        addFlags: ['c3-route-healer'],
        result:
          'You send Lysara with Elene and help Mara push the first wagon through the frightened crowd.',
      },
      {
        id: 'c3-focus-road',
        label: 'Hear Varris’s offer without giving him the iron.',
        detail:
          'Look for an escape route while keeping the evidence in your hands.',
        next: 'c3-broker',
        changes: { command: 1 },
        addFlags: ['c3-route-broker'],
        result:
          'You make Tivik hold the wrapped iron while Varris leads you into a shop with six doors and no windows.',
      },
    ],
  },
    ...
    choices: [
      {
        ...
        result:
          'You catch a young clerk with a Crown seal in his pocket. He swears a royal courier paid him to leave the ledger open.',
      },
      {
        ...
        result:
          'Lysara copies seven locations for one bridge. None of them agree with Harrowfen’s current maps.',
      },
      {
        ...
        result:
          'Lysara touches the page. The ink forms the outline of a man carrying your sword but wearing a royal courier’s silver gloves.',
      },
    ],
  },
    ...
    choices: [
      {
        ...
        result:
          'You take the stones on your shield until Elene’s guards clear the bridge. No wounded traveller is touched.',
      },
      {
        ...
        result:
          'Nilo tells Senna how he earned the scar on his thumb. The dead copy had no scar. Senna opens every bed she has.',
      },
      {
        ...
        result:
          'Brann forms a calm line and gives the crowd room to retreat. The shouting loses its courage when nobody pushes back.',
      },
    ],
  },
    ...
    choices: [
      {
        ...
        result:
          'Your blood falls through the sunny doorway and lands behind you. The route is a loop wearing Greyhaven’s face.',
      },
      {
        id: 'c3-offer-the-wrapping',
        label: 'Give Varris the iron’s stained wrapping.',
        detail:
          'Trade a trace of its power for his map while keeping the fragment.',
        next: 'c3-bill',
        changes: { resolve: 1 },
        addFlags: ['c3-varris-map'],
        result:
          'Varris presses the cloth to a blank map. A bridge appears east of town, then slides away whenever the fragment moves.',
      },
      {
        id: 'c3-seize-mask',
        label: 'Pull off Varris’s mask when your voice speaks again.',
        detail:
          'Act before the imitation finishes distracting you. Requires 2 Resolve.',
        next: 'c3-bill',
        requires: { resolve: 2 },
        changes: { resolve: -1, command: 1 },
        addFlags: ['c3-unmasked-varris'],
        result:
          'The mask comes away. Varris has no mouth, only moving ink that shapes your signature across his skin. He points in terror toward the watch house.',
      },
    ],
  },
    ...
    choices: [
      {
        ...
        result:
          'The date stops moving only when the iron points east, toward the old watch house and the bridge beyond it.',
      },
      {
        id: 'c3-ask-town-memory',
        label: 'Ask witnesses for one detail they all remember.',
        detail:
          'Use ordinary questions to find the solid fact. Gain 1 Command.',
        next: 'c3-evidence',
        changes: { command: 1 },
        addFlags: ['c3-witness-pattern'],
        result:
          'Every witness remembers silver gloves. Faces and names change, but the false escort’s courier always wore them.',
      },
      {
        ...
        result:
          'The fragment spins once, cuts a line into the stall, and stops toward the old watch house. A royal seal shines in a high window.',
      },
    ],
  },
  ...
  'c3-evidence': {
    ...
    objective:
      'Identify the royal courier before he learns you are following him.',
    threat: 'Rising',
    art: 'harrowfen',
    body: (state) => [
    ...
    choices: [
      {
        ...
        result:
          'You pull the board free. The burial date changes in your hand, but Ordan’s name remains.',
      },
      {
        ...
        result:
          'You cross the canal bridge before the first burning beam falls and see Ordan enter a room with no outer door.',
      },
      {
        ...
        result:
          'Horns sound at each gate. Ordan cannot leave by an ordinary road, which tells you what kind of escape he needs.',
      },
    ],
  },
    ...
    choices: [
      {
        ...
        result:
          'You fold the list into your coat. The names stay fixed after you carry it away from the cold door.',
      },
      {
        ...
        result:
          'The blood is warm. A cut in the cloth matches no weapon you know, but a scrap of silver glove is caught inside it.',
      },
      {
        ...
        result:
          'The cold door catches fire. Outside, three people suddenly remember that the false escort never entered their homes.',
      },
    ],
  },
    ...
    choices: [
      {
        ...
        detail:
          'Make your tactical priority clear. Agreement may build trust, but it is not a romantic promise.',
        next: 'c3-market-memory',
        addFlags: ['c3-priority-people'],
        result:
          'Mara’s shoulders loosen. She touches your wrist and says, “Then make sure you come back as one of them.”',
      },
      {
      ...
      },
      {
        ...
        detail:
          'Requires 2 Command. Spend 1 Command so protection and investigation support each other.',
        next: 'c3-market-memory',
        requires: { command: 2 },
        changes: { command: -1 },
        addFlags: ['c3-balanced-plan'],
        result:
          'Mara takes the wounded. Lysara watches the iron. Their hands meet over your map, and the argument becomes a plan.',
      },
    ],
  },
    ...
    choices: [
      {
        ...
        detail:
          'Spend 2 Command to give the crowd one place and one instruction.',
        next: 'c3-pin-test',
        requires: { command: 2 },
        changes: { command: -2 },
        addFlags: ['c3-saved-market-crowd'],
        result:
          'Your guards repeat the order until frightened voices become one moving line. The old well remains in every version of the square.',
      },
      {
        ...
        result:
          'The road becomes a canal under your final step. Mara catches your arm and pulls all three of you onto fixed stone.',
      },
      {
        id: 'c3-follow-tiviks-rule',
        label: 'Order everyone to stop moving the iron.',
        detail:
          'Use what you learned and make the fragment hold one version in place.',
        next: 'c3-pin-test',
        changes: { resolve: 1 },
        addFlags: ['c3-stilled-fragment'],
        result:
          'Tivik lowers the fragment onto the well. The market stops changing, though cries continue from streets trapped in other versions.',
      },
    ],
  },
    ...
    choices: [
      {
        ...
        result:
          'You cross the fixed circle with empty hands. Your double watches you with the hatred of a man who knows your next mistake.',
      },
      {
        ...
        result:
          'Mara vanishes into the moving streets. A moment later, her knife appears against your double’s back.',
      },
      {
        id: 'c3-use-oath-sight',
        label: 'Use Oathfire to find which person made a real promise.',
        detail:
          'Spend 1 Oathfire. A true promise leaves heat that a copied memory cannot create.',
        next: 'c3-duplicate',
        requires: { oathfire: 1 },
        changes: { oathfire: -1 },
        addFlags: ['c3-saw-false-oath'],
        result:
          'Fire traces the promise Mara made at Bellweather. The woman beside your double has no heat at all. She is a road memory, not your companion.',
      },
    ],
  },
    ...
    choices: [
      {
        ...
        result:
          'Your double lowers the knife. “Then do better than I did,” he says before the rain takes him.',
      },
      {
        id: 'c3-disarm-double',
        ...
        result:
          'You strike the knife away. The double grips your sleeve and whispers that Ordan is already beside the well.',
      },
      {
        ...
        result:
          '“Do not trust the first end of the bridge,” he says. “And the thief is not working for Ordan.” Then his face becomes mist.',
      },
    ],
  },
    ...
    choices: [
      {
        ...
        result:
          'Your blade opens the glove and Ordan’s palm. He keeps the fragment, but his blood marks the route he uses to escape.',
      },
      {
        id: 'c3-order-volley',
        ...
        result:
          'Arrows strike in a clean ring. Ordan’s road cracks, forcing him to run across the unstable edge instead of vanishing at once.',
      },
      {
        id: 'c3-mark-ordan',
        ...
        result:
          'The bill wraps around Ordan’s coat. Its ink burns bright and draws a black line through every road he crosses.',
      },
    ],
  },
    ...
    choices: [
      {
        ...
        detail:
          'Spend 2 Health. Ordan gains distance, but the wounded survive.',
        next: 'c3-pursuit',
        requires: { health: 2 },
        changes: { health: -2 },
        addFlags: ['c3-saved-healing-house'],
        result:
          'You brace a fading support post until Mara carries the last patient out. The building vanishes the moment you let go.',
      },
      {
        ...
        result:
          'Elene takes the square. You, Mara, and Lysara follow Ordan’s marked route before it can close.',
      },
      {
        id: 'c3-oath-hold-harrowfen',
        label: 'Swear that Harrowfen will not be erased today.',
        detail:
          'Spend 2 Resolve. Gain 3 Oathfire, but this promise binds you until the town is stable.',
        next: 'c3-pursuit',
        requires: { resolve: 2 },
        changes: { resolve: -2, oathfire: 3 },
        addFlags: ['c3-oath-hold-town'],
        result:
          'Fire fills the cracks between streets. Harrowfen stops fading, and your new promise burns whenever Ordan moves the fragment.',
      },
    ],
  },
    ...
    choices: [
      {
        ...
        result:
          'Mara fastens your sword belt herself. “Try not to meet another you,” she says. “One is enough trouble.”',
      },
      {
        ...
        result:
          'Lysara grows a green thread across the stones. She warns that it will show a way back only once.',
      },
      {
        id: 'c3-use-oath-trail',
        label: 'Use Oathfire to mark Ordan’s direction.',
        detail:
          'Spend 1 Oathfire. Your promise creates a trail across changing distance.',
        next: 'c3-world-nail',
        requires: { oathfire: 1 },
        changes: { oathfire: -1, command: 1 },
        addFlags: ['c3-oath-trail'],
        result:
          'A red line burns across one arch, vanishes, and returns on another. The bridge cannot hide the direction Ordan took.',
      },
    ],
  },
    ...
    return [
      id,
      {
        ...
        choices: node.choices.map((choice) => ({
          ...
          advantage:
            adventureChoiceUpdates[choice.id]?.advantage ??
            choice.advantage ??
...
export function canChoose(choice: Choice, state: GameState) {
  const hasStats =
    !choice.requires ||
    Object.entries(choice.requires).every(
      ([key, value]) => state.stats[key as StatKey] >= (value ?? 0),
    );
  const hasRelationships =
    !choice.requiresRelationships ||
    Object.entries(choice.requiresRelationships).every(([person, needs]) => {
      const score = state.relationships[person as RelationshipKey];
      return (
        (!needs?.trust || score.trust >= needs.trust) &&
        (!needs?.attraction || score.attraction >= needs.attraction) &&
        (!needs?.respect || score.respect >= needs.respect) &&
        (!needs?.intent || score.intent === needs.intent)
      );
    });
  const hasFlags =
    !choice.requiresFlags ||
    choice.requiresFlags.every((flag) => state.flags.includes(flag));
  const isVisible = isChoiceVisible(choice, state);
  const avoidsForbiddenIntent =
    !choice.forbidsRelationshipIntents ||
    Object.entries(choice.forbidsRelationshipIntents).every(
      ([person, intents]) =>
        !intents?.includes(
          state.relationships[person as RelationshipKey].intent,
        ),
    );
  return (
    isVisible &&
...
export function isChoiceVisible(choice: Choice, state: GameState) {
  ...
  const avoidsHiddenFlags =
    !choice.hideIfAnyFlags ||
    !choice.hideIfAnyFlags.some((flag) => state.flags.includes(flag));
  const matchesRelationshipIntents =
    !choice.showIfRelationshipIntents ||
    Object.entries(choice.showIfRelationshipIntents).every(
      ([person, intents]) =>
        intents?.includes(
          state.relationships[person as RelationshipKey].intent,
        ),
  ...
  const respectsResolvedRelationships = Object.entries(effects).every(
    ([person, changes]) => {
...
export function requirementText(choice: Choice) {
  ...
  for (const [person, needs] of Object.entries(
    choice.requiresRelationships ?? {},
  )) {
    const name = relationshipLabels[person as RelationshipKey];
    if (needs?.trust && needs?.attraction)
      parts.push(`a strong mutual bond with ${name}`);
    else if (needs?.trust) parts.push(`established trust with ${name}`);
    else if (needs?.attraction)
      parts.push(`acknowledged attraction with ${name}`);
    if (needs?.respect) parts.push(`established respect with ${name}`);
    if (needs?.intent === 'committed') parts.push(`commitment to ${name}`);
    if (needs?.intent === 'exploring')
      parts.push(`an openly explored relationship with ${name}`);
    if (needs?.intent === 'platonic') parts.push(`friendship with ${name}`);
  }
  for (const [person, intents] of Object.entries(
    choice.forbidsRelationshipIntents ?? {},
  )) {
    if (intents?.length)
      parts.push(
        `an honest resolution with ${relationshipLabels[person as RelationshipKey]}`,
      );
  }
