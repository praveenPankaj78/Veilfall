...
Caelan keeps the real Distance fragment after the Mileless Bridge, reaches Dragonspine, becomes the bearer of Vaor's ember through freedom, force, or pact, earns or refuses steppe and Crown forces, and defends the Black Gate through one of three strategies. He controls how Vexa's embassy is first heard, recovers the missing Black Gate Nail piece through bargain, theft, or exposure, crosses into the Cinder Deep, and reaches the inner Gate through revolt, auction, or force. He then writes one of four Gate laws and separately chooses his destination and relationship ending. Rook's departure cannot remove any of these decisions.
...
Exact post-implementation contracts are also machine readable. Rook begins the Underways with only his last mirrored coin if two were destroyed on the bridge. A destroyed Dragonspine drill must be rebuilt before any later extraction. Vaor's memory-repair duty must be paid before later memory, testimony, or power cooperation. Preserved ancestor voices must be tested in the first Unsea investigation. A Kharad service debt gives the Red Moot one bounded post-Gate claim before the Crown can use it, and archived Orivane proof gives an independent Kharad custodian standing at the first Crown hearing. Hale's last-known-alive record proves neither death nor survival. Chapter Nine preserves Vexa's exact entry location and whether Ansel already received the first question. Chapter Eleven actively consumes the exact Gate Nail recovery route, true-name access limit, surviving Oaths, evidence provenance, Vexa relationship, Vaor state, Ash Road method, Free Ledger price, and ally roster. Chapter Twelve consumes the opening Gate, chosen Vathis alliance, bounded Elian evidence, and each mythic freedom restriction. Its terminal export carries the exact Gate law, stored-promise result, Caelan destination, relationship conclusion, fragment custody, transformed state, changed sunrise, and later Rook approach.
